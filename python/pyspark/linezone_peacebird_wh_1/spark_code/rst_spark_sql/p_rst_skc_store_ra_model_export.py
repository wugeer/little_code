...
# Filename: p_rst_skc_store_ra_model_export
# Author: liuhai
...
if __name__ == '__main__':
    ...
    sql_sales_1 = '''
        select a.product_code,a.color_code,a.size_code                --sku
            ...
            , a.last2_seven_days_sales_qty                          -- 上上7天销量
            , a.last_thirty_days_sales_qty                          -- 过去30天销量                     
            , a.total_sales_qty
    ...
    sql_sales_2 = '''
        select product_code,color_code,store_code,size_code,day_date,total_sales_qty
            ...
            ,concat(size_name,'+',last2_seven_days_sales_qty,'+',size_code) as last2_seven_days_sales_qty
            ,concat(size_name,'+',last_thirty_days_sales_qty,'+',size_code) as last_thirty_days_sales_qty
    ...
    sql_sales_3 = '''
        select product_code,color_code,store_code,day_date
            ...
            ,concat_ws(',',collect_set(last2_seven_days_sales_qty)) as last2_seven_days_sales_qty
            ,concat_ws(',',collect_set(last_thirty_days_sales_qty)) as last_thirty_days_sales_qty
    ...
    sql_replenish_1 = '''
        ...
        --inner join {p_edw_schema}.dim_store h on a.send_org_id = h.store_id
    ...
    #特别注意 此处将补货的 发货组织和收货组织 调换位置 ，并将补货量 置为 负数
    sql_replenish_2 = '''
        select 
            a.product_code,a.color_code,a.size_code
            ,a.receive_org_code as send_org_code  -- 将发货组织 和 收货组织 调换 要求补货 置为负数
            ,a.send_org_code as receive_org_code
            ,concat(a.size_name,'+',-1*a.suggest_replenish_qty,'+',a.size_code) as suggest_replenish_qty   
            ,a.suggest_replenish_qty as suggest_replenish_qty_int
            ,a.day_date
        from replenish_1 a
    '''
    ...
    sql_allot_1 = '''
        ...
        inner join {p_edw_schema}.dim_target_store s 
            on a.send_store_id = s.store_id
        inner join {p_edw_schema}.dim_target_store h 
            on a.receive_store_id = h.store_id
        inner join {p_edw_schema}.dim_product_sku c 
            on a.product_id = c.product_id 
            and a.color_id = c.color_id
            and a.size_id = c.size_id
    ...
    sql_return_1 = '''
        select c.product_code,c.color_code,c.size_code                --sku
            ...
            ,s.store_code as send_org_code                            --发货组织
            --,h.store_code as receive_org_code                            --收货组织
            ,'CB11' as receive_org_code                            --收货组织
            ,a.send_qty as suggest_return_qty    --返仓数量
            --, (case 
            --        when mcr.color_id is not null then '返仓'
            --        else null
            --    end ) as move_type                    -- 新增返仓标记
            ,'{p_input_date}' as day_date
        ...
        inner join {p_edw_schema}.dim_product_sku c on a.product_id = c.product_id and a.color_id = c.color_id
            and a.size_id = c.size_id
        --left join {p_edw_schema}.mod_skc_return  mcr                             --mod_skc_return 表里面 SKC+门店的组合出现了，则标记为返仓，否则默认为空
        --    on a.send_store_id = mcr.store_id
        --        and a.product_id = mcr.prod_id and a.color_id = mcr.color_id 
        where a.dec_day_date = date_add('{p_input_date}',1) 
    ...
    sql_return_3 = '''
        select 
            ...
            ,send_org_code,receive_org_code
            ,day_date
    ...
    sql_all_rep_ret = '''   
        ...
        select 
            ...
            ,send_org_code,receive_org_code
            ,day_date
            ,suggest_replenish_qty as suggest_allot_qty
        from replenish_3
        ...
        select 
            product_code,color_code
            ,send_org_code,receive_org_code,day_date
            ,suggest_return_qty as suggest_allot_qty
        from return_3
    
    '''

    sql_insert = '''
        insert overwrite table {p_rst_schema}.rst_skc_store_ra_model_export partition(day_date)
        --insert overwrite table rst.rst_skc_store_ra_model_export partition(day_date)
        select after_ra_4.product_code                      --产品编码
            ...
            , coalesce(sales_3.last2_seven_days_sales_qty,0) as last2_seven_days_sales_qty   -- 上山7天销量，没有补零
            , coalesce(sales_3.last_thirty_days_sales_qty,0) as last_thirty_days_sales_qty    --过去30天销量，没有补零
            ...
            , null as suggest_replenish_qty                --建议补货量， null
            , null as suggest_replenish_qty_int
            , all_rep_ret.suggest_allot_qty                    --建议调出量，null
            , null as suggest_allot_qty_int
            , null as suggest_return_qty                --建议返仓量，null
            , null as replenish_allot_qty            --skc的建议补调量 没有用到null
            , after_ra_4.replenish_allot_after_qty as replenish_allot_after_stock           --补调后库存（加在途）
            , (case 
                    when mcr.color_id is not null then '返仓'
                    else null
                end ) as move_type                    -- 新增返仓标记
            , all_rep_ret.receive_org_code              --调入门店编码
            --, hhh.org_name as receive_org_name              --调入门店名称
            , (case when all_rep_ret.receive_org_code = 'CB11' then '退货仓'
                    when all_rep_ret.receive_org_code = 'CB37' then '补货仓'
                    else hhh.org_name
               end) as receive_org_name              --调入门店名称
            --, hhh.org_longcode as receive_org_long_code     --调入门店长编码
            , (case when all_rep_ret.receive_org_code = 'CB11' then '退货仓'
                    when all_rep_ret.receive_org_code = 'CB37' then '补货仓'
                    else hhh.org_longcode
               end) as receive_org_long_code     --调入门店长编码
        ...
        left join {p_edw_schema}.mod_skc_return  mcr                             --mod_skc_return 表里面 SKC+门店的组合出现了，则标记为返仓，否则默认为空
            on  mcr.day_date = date_add('{p_input_date}',1)
                and store.store_id = mcr.store_id
                and pc.product_id = mcr.prod_id and pc.color_id = mcr.color_id
        ...
        left join all_rep_ret 
            on after_ra_4.product_code = all_rep_ret.product_code 
            and after_ra_4.color_code = all_rep_ret.color_code 
            and after_ra_4.store_code = all_rep_ret.send_org_code
        left join {p_edw_schema}.dim_stockorg hhh 
            on all_rep_ret.receive_org_code = hhh.org_code
    ...
    spark.create_temp_table(sql_all_rep_ret, 'all_rep_ret')
    ...
    spark.drop_temp_table('all_rep_ret')
