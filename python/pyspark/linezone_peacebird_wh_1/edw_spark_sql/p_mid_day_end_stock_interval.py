...
class MidDayEndStocks:
    ...
    def __init__(self):
        ...
        # 理想情况下,task数量设置成Spark Application 的总CPU核数,但是现实中总有一些task运行慢一些task快,导致快的先执行完
        # ,空余的cpu 核就浪费掉了,所以官方推荐task数量要设置成Spark Application的总cpu核数的2~3 倍
        spark_conf.set("spark.default.parallelism", "150")
        spark_conf.set("spark.driver.memory", "30g")
        # 构造sparksession对象
...
if __name__ == '__main__':
    ...
    # 计算每天的库存
    res_dic = {"p_begin_date": repr(p_begin_date.strftime("%Y-%m-%d")),
               "p_end_date": repr(p_end_date.strftime("%Y-%m-%d"))}
    sql_day_end_stock = """
        ...
        where stock_date < {p_begin_date} or stock_date > {p_end_date}
        """.format(**res_dic)

    # 执行入库操作
    # midDayEndStocks.spark_session.sql(sql_day_end_stock)
