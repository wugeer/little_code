...
class bs4_test(object):
    def __init__(self,origin_url):
        ...
        time.sleep(1)
        soup = bs4.BeautifulSoup(res.text, 'html.parser')
        linkElems = soup.select("ul.clearfix a")[14:-20]
        titleElems = soup.select("ul.clearfix a p")[5:-20]
        #pprint.pprint(titleElems)
        for i in range(len(linkElems)):
            #print(linkElems[i].get("href"))
            self.pre_download_bs4(linkElems[i].get("href"), titleElems[i].get_text())
        #pprint.pprint(linkElems)
        #for i in range(len(linkElems)):
    ...
    def pre_download_bs4(self, total_url, path):
        ...
        for i in range(int(len_)):
            download_url = "".join([total_url[:-5], "_" + str(i + 1), total_url[-5:]])
            self.downloader_base_bs4(download_url, path, i + 1)
        # print(len_)
    def downloader_base_bs4(self, base_url, path, index):
        """
        单线程下载传入的每个主题的图片
...
if __name__ == '__main__':
    ...
    url = "http://www.win4000.com/meinvtag2.html"
    #xs_obj = meizhuo(url)
    for i in range(5):
        xs_obj = bs4_test("".join([url[:-5], '_', str(i+1), '.html']))