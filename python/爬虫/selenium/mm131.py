...
class mm131(object):
    ...
    def downloader_base_bs4(self, base_url, path, index):
        ...
        #     text)
        # #print(len_)
        # pprint.pprint([i.find_element_by_tag_name("img").get_attribute("src") for i in self.chrome_browser.
        #               find_elements_by_css_selector("[class='pic-meinv']")])
        # pprint.pprint([i.get_attribute("src") for i in self.chrome_browser.
        #               find_elements_by_tag_name("img")])
        # url1 = self.chrome_browser.find_element_by_css_selector("[class='pic-large']").get_attribute("src")
        # pprint.pprint(url1)
        # if not os.path.exists(dir):
        #     os.mkdir(dir)
        # #path = "".join(["1",])
        ...
        # for i in range(1,len_):
        #     # 每一张图片的URL
        #     download_url = "".join([total_url[:-11],"_" + str(i+1), total_url[-5:]])
        #     print(download_url)
...
class mm7160_bs4(object):
    def __init__(self,origin_url):
        res = requests.get(origin_url, headers=headers)
        self.url = origin_url
        if res.status_code != 200:
            print('the unvalid url %s' % (origin_url))
            return
        res.encoding = 'gb2312'
        time.sleep(1)
        soup = bs4.BeautifulSoup(res.text, 'html.parser')
        #linkElems = soup.select("ul.new-img a")
        linkElems = [("".join([origin_url, item.get("href").split("/")[-1]]), item.get("title")) for item in soup.select("ul.new-img a")]
        #pprint.pprint(linkElems)
        #titleElems = [i.get("alt") for i in soup.select("div.main dd img") if i.get("alt")]
        #pprint.pprint(titleElems)
        #pprint.pprint(titleElems)
        pool = multiprocessing.Pool(processes = 4) 
        for i in range(len(linkElems)):
        #    #print(linkElems[i].get("href"))
            #print(linkElems[i][1])
            # self.pre_download_bs4(linkElems[i][0], linkElems[i][1])
            pool.apply_async(self.pre_download_bs4, (linkElems[i][0], linkElems[i][1], ))
        #pprint.pprint(linkElems)
        #for i in range(len(linkElems)):
        #link = [i for i in linkElems]
        pool.close()
        pool.join()  
    def pre_download_bs4(self, total_url, path):
        print(total_url)
        ...
        time.sleep(1)
        soup = bs4.BeautifulSoup(res.text, 'html.parser')
        linkElem =["".join([total_url[:-5], "_", item.get_text(), '.html']) for item in soup.select("div.itempage a")[3:-1]]
        #linkElem = [i.get("href") for i in soup.select("div.content-page a")[:-1]]
        linkElem.insert(0, total_url)
        #pprint.pprint(linkElem)
        #len_ = re.findall("\d+", soup.find('span', {'class': 'page-ch'}).get_text())[0]

        index = 0
        if not os.path.exists("mm7160"):
            os.mkdir("mm7160")
        path = "".join(["mm7160/", path])
        if not os.path.exists(path):
            os.mkdir(path)
        #print(path)
        for i in linkElem:
            #download_url = "".join([self.url, i])
            index += 1
            #print(i)
            self.downloader_base_bs4(i, path, index)
        # print(len_)
    def downloader_base_bs4(self, base_url, path, index):
        """
        单线程下载传入的每个主题的图片
        ...
        time.sleep(1)
        soup = bs4.BeautifulSoup(res.text, 'html.parser')
        #print(soup.select("div.picsbox.picsboxcenter a"))
        url = soup.select("div.picsbox.picsboxcenter img")[0].get("src")
        # pprint.pprint(linkElems)
        # print('There are %s Picture waiting for downloading:\n' % (len(linkElems)))
        # for i in range(len(linkElems)):
        #if not url:
        ...
        if res1.status_code != 200:
            print("cann't download " + url)
            return
        with open("".join([path, '/', str(index), '.jpg']), 'wb') as f:
            f.write(res1.content)
            print("写入成功")
